...
bestf=[];
for cellnum=1:12
    load('D:\Dropbox\cellremoval2.mat')
    %this file should contain 3 variables
    %T=timesteps in data
    %N=number of cells
    %T x 1 vector i_total for battery current
    %T x 1 vector t_total for time in seconds
    %T x N vector v_total for battery cell voltages
    
    time = t_total(startt:1:5.2*10^4)/60/60; %time in hours
...
totalpossiblecap=sum(maxc)
usablecap=12*min(maxc)
cappercell=maxc
% *
